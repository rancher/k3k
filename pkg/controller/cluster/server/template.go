...
safe_mode() {
	...
	if [ -d "{{.ETCD_DIR}}" ]; then
		echo "[K3k-Boot] Starting K3s in Safe Mode (Network Policy Disabled) to patch Node IP from ${CURRENT_IP} to ${POD_IP}"
		/bin/k3s server --disable-network-policy --config $1 {{.EXTRA_ARGS}} > /dev/null 2>&1 &
		PID=$!

		# Start the loop to wait for the nodeIP to change
		echo "[k3k-boot] Waiting for Node IP to update to ${POD_IP}..."
		count=0
		until kubectl get nodes -o wide 2>/dev/null | grep -q "${POD_IP}"; do
			if ! kill -0 $PID 2>/dev/null; then
				echo "[K3k-Boot] FATAL: Safe Mode K3s process died unexpectedly!"
				exit 1
			fi
			sleep 2
			count=$((count+1))
			...
			if [ $count -gt 60 ]; then
				echo "[K3k-Boot] FATAL: Timed out waiting for node to change IP from $CURRENT_IP to $POD_IP"
				exit 1
			fi
		done
		
		echo "[k3k-boot] Node IP is set to ${POD_IP} successfully. Stopping Safe Mode process..."
		kill $PID
...
single_server() {
	echo "[k3k-boot] Starting single node setup..."

	# checking for existing data in single server if found we must perform reset
	if [ -d "{{.ETCD_DIR}}" ]; then
		echo "[k3k-boot] Existing data found in single node setup. Performing cluster-reset to ensure quorum..."

		if ! /bin/k3s server --cluster-reset --config {{.INIT_CONFIG}} {{.EXTRA_ARGS}} > /dev/null 2>&1; then
			echo "[k3k-boot] Cluster reset failed!"
			exit 1
		fi
		echo "[k3k-boot] Cluster reset complete. Removing Reset flag file."
	...
	echo "[k3k-boot] Adding pod IP file."
...
ha_server() {
	echo "[k3k-boot] Starting pod $POD_NAME in HA node setup"
