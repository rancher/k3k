...
|===
| Field | Description | Default | Validation
| *`clusterName`* __string__ | ClusterName is the name of the cluster where the snapshot will be taken + |  | MinLength: 1 +

| *`s3ConfigSecretRef`* __link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#secretreference-v1-core[$$SecretReference$$]__ | S3ConfigSecret defines the S3 configuration secret that contains all +
...
| *`dir`* __string__ | Dir defines the location where the snapshot will be created, if +
left empty k3k will use the directory configured for k3s when the +
cluster was created + |  | 
| *`compress`* __boolean__ | Specifies if the snapshot should be compressed + |  | 
|===
...
|===
| Field | Description | Default | Validation
| *`location`* __string__ | Location is the absolute file:// or s3:// URI address of the snapshot. + |  | 
