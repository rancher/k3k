...
| *`s3ConfigSecret`* __link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#secretreference-v1-core[$$SecretReference$$]__ | S3ConfigSecret defines the S3 configuration secret that contains all +
...
| *`snapshotDir`* __string__ | SnapshotDir defines the location where the snapshot will be created, if +
left empty k3k will use the directory configured for k3s when the +
cluster was created + |  | 
| *`snapshotCompress`* __boolean__ | SnapshotCompress specifies if the snapshot should be compressed + |  | 
|===
...
|===
| Field | Description | Default | Validation
| *`address`* __string__ | Address is the absolute file:// or s3:// URI address of the snapshot. + |  | 
| *`size`* __link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#quantity-resource-api[$$Quantity$$]__ | Size is the size of the snapshot file, in bytes. If not specified, the snapshot failed. + |  | 
| *`creationTime`* __link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#time-v1-meta[$$Time$$]__ | CreationTime is the timestamp when the snapshot was taken by etcd. + |  | 
| *`readyToUse`* __boolean__ | ReadyToUse indicates that the snapshot is available to be restored. + |  | 
| *`error`* __xref:{anchor_prefix}-github-com-k3s-io-api-k3s-cattle-io-v1-etcdsnapshoterror[$$ETCDSnapshotError$$]__ | Error is the last observed error during snapshot creation, if any. +
